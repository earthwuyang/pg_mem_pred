...
class EnhancedTransformerConv(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels, heads=4, dropout=0.5):
        super(EnhancedTransformerConv, self).__init__()
        self.conv1 = TransformerConv(num_node_features, hidden_channels // heads, heads=heads, dropout=dropout)
...
model = EnhancedTransformerConv(num_node_features, hidden_channels)
print(model)
...
for epoch in range(1, epochs + 1):
    ...
    # Save model
    torch.save(model.state_dict(), f'model_EnhancedTransformerConv.pth')
