...
def train_model(logger, args):
    ...
    sample_graph = testdataset[0]
    num_operator_features = sample_graph.x_dict['operator'].shape[1]
    num_table_features = sample_graph.x_dict['table'].shape[1]
    num_column_features = sample_graph.x_dict['column'].shape[1]
    num_predicate_features = sample_graph.x_dict['predicate'].shape[1]
    num_operation_features = sample_graph.x_dict['operation'].shape[1]
    num_literal_features = sample_graph.x_dict['literal'].shape[1]
    num_numeral_features = sample_graph.x_dict['numeral'].shape[1]


    # output dim is 2 for peakmem and time
    model = HeteroGraph(args.hidden_dim, 1, args.num_layers, 
                        num_operator_features, num_table_features, num_column_features, num_predicate_features, 
                        num_operation_features, num_literal_features, num_numeral_features)
