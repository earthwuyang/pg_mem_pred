...
def load_schema_json(dataset):
    schema_path = os.path.join(os.path.dirname(__file__), '../../cross_db_benchmark/datasets/', dataset, 'schema.json')
...
def load_column_statistics(dataset, namespace=True):
    path = os.path.join(os.path.dirname(__file__), '../../cross_db_benchmark/datasets/', dataset, 'column_statistics.json')
...
def load_string_statistics(dataset, namespace=True):
    path = os.path.join(os.path.dirname(__file__), '../../cross_db_benchmark/datasets/', dataset, 'string_statistics.json')
    assert os.path.exists(path), f"Could not find file ({path})"
    return load_json(path, namespace=namespace)
...
def load_schema_sql(dataset, sql_filename):
    sql_path = os.path.join(os.path.dirname(__file__), '../../cross_db_benchmark/datasets/', dataset, 'schema_sql', sql_filename)
