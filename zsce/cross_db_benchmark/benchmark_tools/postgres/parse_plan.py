...
def parse_recursively(parent, plan, offset, depth):
    ...
    while i < len(plan):
        ...
        if plan[i].strip().startswith('->'):
            ...
            # create plan node for previous one. wuy: because we now meet a new operator
            ...
            if new_depth > depth:
                ...
                i = parse_recursively(operators_current_level[-1], plan, i, new_depth) # new level: deeper
...
def parse_plan(analyze_plan_tuples, analyze=True, parse=True):
    ...
    if analyze:
        for i, plan_step in enumerate(plan_steps):
            ...
            ex_time_match = planning_time_regex.match(plan_step)  # planning_time_regex=re.compile('planning time: (?P<planning_time>\d+.\d+) ms')
            if ex_time_match is not None:
                planning_idx = i
                planning_time = float(ex_time_match.groups()[0])  

            ex_time_match = ex_time_regex.match(plan_step)  # ex_time_regex=re.compile('execution time: (?P<execution_time>\d+.\d+) ms')
            if ex_time_match is not None:
...
def parse_plans(run_stats, min_runtime=0, max_runtime=30000, parse_baseline=False, cap_queries=None,
    ...
    for q in tqdm(run_stats.query_list): # query_list is a list of {'analyzed_plans', 'verbose_plan', 'sql'}
        ...
        if not curr_explain_only:
            ...
            for analyze_plan in q.analyze_plans:
                _, ex_time, planning_time = parse_plan(analyze_plan, analyze=True, parse=False) # parse=False means we only get ex_time and planning_time
            ...
            # currently analyze_plan is only {}...{} with depth information
            # print(f"\nplan_content: {analyze_plan.plain_content}") # the line of the root operator (the first line of all lines)
        ...
        # verbose_plan before parse is only {}...{}, only a shape
        ...
        if not curr_explain_only:
            ...
            analyze_plan.merge_recursively(verbose_plan)  # add output_columns from verbose_plan to analyze_plan.
        ...
        if parse_baseline:
            ...
            analyze_plan.peakmem = q.peakmem
        ...
        #     print(f"Error parsing query {q.sql}: {e}")
        #     continue
    ...
    print(f"Parsed {len(parsed_plans)} plans ({len(run_stats.query_list) - len(parsed_plans)} had zero-cardinalities "
          f"or were too fast or because query number is capped).")
