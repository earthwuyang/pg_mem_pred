...
# filter_columns_regex = re.compile('("\S+"."\S+")')  # \S is used to match any non-blank character
...
class PlanOperator(dict):
    ...
    def parse_lines(self, alias_dict=None, parse_baseline=False, parse_join_conds=False):
        op_line = self.plain_content[0] # [0] is used to fetch the content of the list with only one element

        # parse plan operator name
        op_name_match = op_name_regex.search(op_line)  # op_name_regex=re.compile('->  ([^"(]+)'), [^"(] matches any character that is not " or (
        assert op_name_match is not None
        op_name = op_name_match.groups()[0]  # the groups() method returns a tuple containing all the captured groups from the match. Since we have only one capturing group `([^"(])`, we access the first (and only) element with `[0]`
        ...
        match_est = estimated_regex.search(op_line) # estimated_regex = re.compile('\(cost=(?P<est_startup_cost>\d+.\d+)..(?P<est_cost>\d+.\d+) rows=(?P<est_card>\d+) width=(?P<est_width>\d+)\)')
        ...
        match_act = actual_regex.search(op_line)  # actual_regex = re.compile('\(actual time=(?P<act_startup_cost>\d+.\d+)..(?P<act_time>\d+.\d+) rows=(?P<act_card>\d+)')
    ...
    def parse_output_columns(self, l):
        ...
        for col in l.split(', '):
            ...
            # for operators to change dates
                ...
                # remove literals
                col = re.sub(literal_regex, '', col)  # literal_regex = re.compile('(\'[^\']+\'::[^\'\)]+)')
                ...
                assert filter_columns_regex.search(col) is not None, f"Could not parse {col}"  # filter_columns_regex = re.compile('([^\(\)\*\+\-\'\= ]+)')

                for filter_m in filter_columns_regex.finditer(col): # return all positions that match the regex
                    curr_col = filter_m.group() # return matched string
                    endpos = filter_m.span()[-1] # span() returns start and end position, [-1] return the end position.
                ...
                col = col.strip('(')
                for curr_agg in list(Aggregator) + list(ExtendedAggregator):  # ['avg', 'sum', 'count', 'min', 'max']
                    if col.startswith(str(curr_agg).lower()):
                        agg = str(curr_agg)

                # assert agg is not None, f"Could not parse {col}"
            output_columns.append(dict(aggregation=str(agg), columns=columns))  # aggregation could be None as I find from sample output
    ...
    def merge_recursively(self, node):
        ...
        if self.plan_parameters['op_name'] != node.plan_parameters['op_name']:
            print(f"warning: in merging different operators {self.plan_parameters['op_name']} != {node.plan_parameters['op_name']}")
