...
def validate_model(model, val_loader, criterion, scalers, device, ):
    # Validation
    model.eval()
    val_loss = 0
    mem_preds = []
...
def train_epoch(logger, model, optimizer, criterion, train_loader, val_loader, early_stopping, epochs, device, scalers, mem_pred, time_pred):
    ...
    for epoch in range(epochs):
        ...
        avg_val_loss, metrics = validate_model(model, val_loader, criterion, scalers, device)
...
def train_model(logger, args):
    ...
    if not args.skip_train:
        traindataset = QueryPlanDataset(logger, dataset_dir, train_dataset, 'train', scalers)
        logger.info('Train dataset size: {}'.format(len(traindataset)))
        valdataset = QueryPlanDataset(logger, dataset_dir, train_dataset, 'val', scalers)
    ...
    testdataset = QueryPlanDataset(logger, dataset_dir, test_dataset, 'test', scalers)  
