...
class BFStrategy:
    ...
    def execute(self) -> float:
        ...
        end_time = time.time()
        total_exec_time = end_time - start_time
        logging.info(f"BF Strategy Total Execution Time: {total_exec_time:.2f} seconds.")
        return total_exec_time
...
def main():
    ...
    if not args.no_ffd:
        ...
        for i in range(args.exp_num):
            ...
            try:
                ffd_based_thread.start()
                memory_based_total_time = ffd_strategy.execute()
                ffd_stop_event.set()
                ffd_based_thread.join()
            except Exception as e:
                logging.error(f"FFD Strategy failed: {e}")
                memory_based_total_time = float('inf')
            ffd_total_time_list.append(memory_based_total_time)

            # Log memory spill after strategy execution
            log_memory_spill(engine, 'FFD')

            # Calculate sum of waiting times
            memory_based_waiting_sum = sum(
                info['total_time'] for info in ffd_strategy.results.values() if 'total_time' in info
            )
            ffd_waiting_sum_list.append(memory_based_waiting_sum)
    ...
    if not args.no_naive:
        save_strategy_results(result_path, bf_strategy.results, ffd_strategy.results, naive_strategy.results)
    ...
    mean_bf_time = mean(bf_total_time_list)
    mean_ffd_time = mean(ffd_total_time_list)
    mean_naive_waiting = mean(naive_waiting_sum_list) if not args.no_naive else 0
    mean_bf_waiting = mean(bf_waiting_sum_list)
    mean_ffd_waiting = mean(ffd_waiting_sum_list)
    ...
    bf_total_retry_count = sum(
        info['retry_count'] for info in bf_strategy.results.values() if'retry_count' in info
    )
    ffd_total_retry_count = sum(
        info['retry_count'] for info in ffd_strategy.results.values() if'retry_count' in info
    )

    naive_max_retry_count = max(
        info['retry_count'] for info in naive_strategy.results.values() if'retry_count' in info
    ) if not args.no_naive else 0
    bf_max_retry_count = max(
        info['retry_count'] for info in bf_strategy.results.values() if'retry_count' in info
    )
    ffd_max_retry_count = max(
        info['retry_count'] for info in ffd_strategy.results.values() if'retry_count' in info
    )
    ...
    if not args.no_ffd:
        logging.info(f"FFD Strategy Average Total Execution Time: {mean_ffd_time:.2f} seconds.")
        logging.info(f"FFD Strategy Average Sum of Waiting Times: {mean_ffd_waiting:.2f} seconds.")
        logging.info(f"Total retries for FFD Strategy: {ffd_total_retry_count}")
        logging.info(f"FFD Strategy Max Retries: {ffd_max_retry_count}\n")

    logging.info(f"BF Strategy Average Total Execution Time: {mean_bf_time:.2f} seconds.")
    logging.info(f"BF Strategy Average Sum of Waiting Times: {mean_bf_waiting:.2f} seconds.")
    logging.info(f"Total retries for BF Strategy: {bf_total_retry_count}")
    logging.info(f"BF Strategy Max Retries: {bf_max_retry_count}\n")

    

    if not args.no_naive and not args.no_ffd:
        if mean_bf_time < mean_ffd_time:
            logging.info("BF Strategy is faster on average.")
        elif mean_bf_time > mean_ffd_time:
            logging.info("FFD Strategy is faster on average.")
        else:
            logging.info("Both strategies have the same average execution time.")
        
        if mean_bf_time < mean_ffd_time:
            logging.info("BF Strategy has a lower average sum of waiting times.")
        elif mean_bf_time > mean_ffd_time:
            logging.info("FFD Strategy has a lower average sum of waiting times.")
        else:
