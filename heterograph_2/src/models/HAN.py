...
class HeteroGraph(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, num_column_features):
        ...
        self.conv1 = HeteroConv({
            ('table', 'scannedby', 'operator'): GATConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('predicate', 'filters', 'operator'): GATConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('column', 'outputby', 'operator'): GATConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('column', 'connects', 'predicate'): GATConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('operator', 'calledby', 'operator'): GATConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('table', 'selfloop', 'table'): GATConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('column', 'selfloop', 'column'): GATConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
        }, aggr='sum')
