...
class PlanNode:
    def __init__(self, node_id, op_name, est_startup_cost, est_cost, est_card, est_width,  workers_planned, est_children_card, statistics):
...
def parse_postgres_plan(plan, statistics, next_node_id=0):
    plan_parameters = plan.get('plan_parameters')
    op_name = plan_parameters.get("op_name", "")
    est_startup_cost = plan_parameters.get('est_startup_cost', 0)
    est_cost = plan_parameters.get('est_cost', 0)
    ...
    # use statistics to normalize the features
    

    node = PlanNode(next_node_id, op_name, est_startup_cost, est_cost, est_card, est_width, workers_planned, est_children_card, statistics)
    next_node_id += 1

    for i,subplan in enumerate(plan.get("children", [])):
        child_node, next_node_id = parse_postgres_plan(subplan, statistics, next_node_id)
        node.add_child(child_node)
