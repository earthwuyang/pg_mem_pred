...
def parse_filter(filter_expression):
    ...
    # Regex for compound filters
    compound_regex = re.compile(r'\(\s*(.*?)\s*(AND|OR)\s*(.*?)\s*\)')
    ...
    # Regex for simple filters
    simple_regex = re.compile(r'\"?(\w+)\"?\s*([<>!=]=?|IS NULL|IS NOT NULL)\s*([\w\.\']+|\d+(\.\d+)?)')
    ...
    if match_simple:
        ...
        return {
            "column": column_id,  # Column ID instead of name
            "operator": operator,
            "literal": literal,
            "literal_feature": None,  # Can be set based on the specific features
