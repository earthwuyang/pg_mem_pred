    ...
    def build_filter_structure(node):
        if isinstance(node, dict):
            if 'and' in node:
                ...
                # Extract the first key which should be the operator
                operator_key = next(iter(node))  # Get the first key
                column_name = node[operator_key][0] if len(node[operator_key]) > 1 else None
                literal = node[operator_key][1] if len(node[operator_key]) > 1 else None
