...
class MscnConv(MessageAggregator):
    ...
    def forward(self, graph=None, etypes=None, in_node_types=None, out_node_types=None, feat_dict=None):
        ...
        with graph.local_scope():
            ...
            # message passing
            graph.multi_update_all({etype: (fn.copy_src('h', 'm'), fn.sum('m', 'ft')) for etype in etypes},  # For each edge type, it copies the source node feature 'h' to the message 'm' and then sums the messages at the destination nodes, storing the result in 'ft'.
                                   cross_reducer='sum')
