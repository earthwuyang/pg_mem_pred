...
class Metric:
    ...
    def evaluate(self, model=None, metrics_dict=None, **kwargs):
        ...
        logging.info(f"{self.metric_name}: {metric:.4f} [best: {self.best_seen_value:.4f}]")
...
class MeanQError(Metric):
    ...
    def evaluate_metric(self, labels=None, preds=None, probs=None):
        ...
        #     qerror.append(max(labels[i], preds[i]) / (min(labels[i], preds[i])+1e-10))
        # qerror = sorted(qerror)
