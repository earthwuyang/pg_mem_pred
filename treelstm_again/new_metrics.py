...
class QError(Metric):
    ...
    def evaluate_metric(self, labels=None, preds=None, probs=None):
        ...
        #     qerror.append(np.maximum(labels[i], preds[i]) / (np.minimum(labels[i], preds[i])+1e-10))
...
def compute_metrics(y_test, y_pred):
    ...
    qerror_50 = QError(percentile=50, min_val=0.1).evaluate_metric(y_test, y_pred)
    qerror_95 = QError(percentile=95, min_val=0.1).evaluate_metric(y_test, y_pred)
    qerror_max = QError(percentile=100, min_val=0.1).evaluate_metric(y_test, y_pred)
    mean_qerror = MeanQError(min_val=0.1).evaluate_metric(y_test, y_pred)
