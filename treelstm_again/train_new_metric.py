...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        # print('Epoch ', epoch, 'Train Loss: ', epoch_loss/len(trainset), ' Validation Loss: ', val_epoch_loss/len(testset), 'MeanQError: ', qerror.item(), 'Mean relative error: ', MRE(dataset, output, memory).item(), 'Qerror_0: ', qerror_0, 'QError_50: ', qerror_50, 'QError_95: ', qerror_95, 'QError_max: ', qerror_max)
        ...
        exp_output = dataset.memory_scaler.inverse_transform(output.cpu().reshape(-1,1)).reshape(-1)
        metrics = compute_metrics(original_memory, exp_output)
        print(f"Metrics: {metrics}")
    torch.save(model.state_dict(), 'plan_net_new_metrics.pth')
