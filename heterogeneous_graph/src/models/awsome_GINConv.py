...
class HeteroGraph(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, num_column_features):
        ...
        # Create a GINConv layer with a neural network
        self.conv1 = HeteroConv({
            ('table', 'scannedby', 'operator'): GINConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('predicate', 'filters', 'operator'): GINConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('column', 'outputby', 'operator'): GINConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('column', 'connects', 'predicate'): GINConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('operator', 'calledby', 'operator'): GINConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('table', 'selfloop', 'table'): GINConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
            ('column', 'selfloop', 'column'): GINConv(nn.Sequential(Linear(hidden_channels, hidden_channels), nn.ReLU(), Linear(hidden_channels, hidden_channels))),
        }, aggr='sum')
